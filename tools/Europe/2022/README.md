...
📝 **Description:** A curated collection of resources for Industrial Control System (ICS) security, including tools, honeypots, research literature, educational materials, and threat intelligence feeds relevant to securing critical infrastructure and SCADA systems.
...
📝 **Description:** An interactive exploitation toolkit that enables penetration testers and red teamers to conduct Azure auditing, discovery, enumeration, and exploitation while reducing operational complexity.
...
📝 **Description:** A tool that executes Emotet botnet modules in isolation for security research, allocating required data structures and invoking custom entry points to enable analysis tasks like debugging and network activity monitoring without infecting the host.
...
📝 **Description:** A tool for exfiltrating data through the DNS protocol via UDP and TCP connections, with configurable query sizes, random delays, and randomized domain names to help avoid detection.
...
📝 **Description:** A Python-based tool that determines whether a system is truly vulnerable to a specific CVE by accounting for runtime conditions, configurations, and mitigations that affect actual exploitability.
...
📝 **Description:** An open-source cloud security platform that automates security assessments and compliance checks across AWS, Azure, GCP, and Kubernetes, providing hundreds of ready-to-use security checks and remediation guidance.
...
📝 **Description:** A Windows shellcode analysis framework that emulates over 20,000 WinAPIs and virtually all Windows syscalls, featuring a custom disassembler capable of revealing deobfuscated code from encoded shellcode.
